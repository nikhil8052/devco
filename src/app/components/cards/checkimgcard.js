...
const Checkimgcard = ({ data, imageWidth, imageHeight }) => {
  return (
    <div className="checkimgcard-sec py-10 md:py-20 mb-24">
      <div className="container max-auto">
        <div className="checkimgcard-container grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-10">
          {data.map((item, index) => (
            <div
              key={index}
              className="w-full checkimgcard rounded-[10px] overflow-hidden shadow-lg border-lightBlue border p-10 data_card bg-white/4 backdrop-blur-lg"
            >
              {/* List Content */}
