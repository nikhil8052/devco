...
const Subheadingtext = ({ heading, subText, list1, list2 }) => {
  ...
  return (
    <div className="sub-heading-ulli-sec pt-5 md:pt-5 relative">
          ...
          {(list1?.length > 0 || list2?.length > 0) && (
            <div className="ulli-text">
              <div className="w-full grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-5 gap-x-4 md:gap-y-10">
                {list1?.length > 0 && (
                  <ul className="section_ulli pl-7 text-opacity-70 max-w-[500px] 2xl:max-w-[600px] list-disc text-[14px] 2xl:text-[20px] xl:text-[18px] md:text-[14px] sm:text-[14px] font-normal text-customwhite md:mb-4">
